...
with mp_hands.Hands(model_complexity=1, max_num_hands=2, min_tracking_confidence=0.7) as hands:
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            # ==========================================
            # [수정된 부분] 역수(1/x) 매핑의 한계 제거 및 3구간 선형 보간 적용
            # 손 크기(hand_size_2d)를 직접 사용하여 앞/뒤 민감도의 밸런스를 맞춥니다.
            # ==========================================
            # 1. 손 크기 기준: [작음(앞), 중간, 큼(뒤)]
            size_points = [50, 130, 250]
            
            # 2. Z축 매핑 
            if not INVERT_Z:
                # 중간 크기(130)일 때의 Z값(400)을 변경하면 앞뒤 밀고 당기는 속도감이 달라집니다.
